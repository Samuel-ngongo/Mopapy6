...
def prever_valor(dados):
    ...
    minimo = max(0.8, estimativa_final - desvio)
    medio = estimativa_final + desvio

    return round(estimativa_final, 2), round(confianca, 1), round(minimo, 2), round(medio, 2)
...
if st.session_state.valores:
    ...
    estimativa, confianca, minimo, medio = prever_valor(st.session_state.valores)
    st.info(f"Estimativa combinada: {estimativa}x")
    st.info(f"Intervalo Inferior: {minimo}x | Intervalo Médio: {medio}x")
